...
export const isSomeKind = (kind: OptionKind): kind is OptionKind.Some =>
...
export function makeOption<T>(kind: OptionKind, value?: T): Option<T> {
    const protoOption = Object.defineProperty(
        (kind === OptionKind.Some
            ? Object.defineProperty({}, VALUE, makeDescriptor(value, false, true))
            : {}
        ), GUARD,
        makeDescriptor(kind, false, true)
    ) as Option<T>;
    Object.defineProperty(protoOption, Symbol.toStringTag, {
        get: () =>
            'Option',
    });
