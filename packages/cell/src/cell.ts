...
export const isCell = (maybeCell: unknown): maybeCell is Cell<unknown> =>
    isObject(maybeCell) && getSymbolFieldValue(maybeCell, GUARD) === CellKind.Cell;
...
export const patch = (cb: PatchFN, configurable = true): void =>
    void patchers.push([cb, configurable]);

const mergeCellWithPatchers = <T>(cell: Partial<Cell<T>>): Cell<T> =>
...
const patchCellWithValue = <T>(cell: Cell<T>) =>
