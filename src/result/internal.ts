...
const implementors: Array<Implementor> = [];
...
export function makeResult<T, E>(kind: ResultKind, value: T | E): Result<T, E> {
    const protoResult = {} as Result<T, E>;
    ...
    Object.defineProperty(protoResult, Symbol.toStringTag, {get: () => 'Result'});
...
function mergeResultReducer<T, E>(kind: ResultKind, value: T | E) {
    return (result: Result<T, E>, [method, whenOk, whenErr, configurable]: Implementor) => (
        Object.defineProperty(result, method, makeDescriptor((kind === ResultKind.Ok
            ? whenOk(value)
            : whenErr(value)
        ), configurable)),
        result
